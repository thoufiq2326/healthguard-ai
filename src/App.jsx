...
const generateMockResult = (query) => {
  ...
  if (lowerQuery.includes('garlic')) {
    ...
    return {
      verdict: "UNCERTAIN",
      score: 42,
      explanation: "[OFFLINE SIMULATION] The claim contains medical terminology but lacks direct peer-reviewed verification in our current clinical index.",
...
const ThreeBackground = ({ activeStep, verdict }) => {
  ...
  useEffect(() => {
    ...
    if (activeStep >= 0 && activeStep < 4) {
      ...
      if (verdict === 'TRUE') { targetColor = 0x10b981; targetEmissive = 0x059669; }
      else if (verdict === 'FALSE') { targetColor = 0xe11d48; targetEmissive = 0xbe123c; }
      else if (verdict === 'MISLEADING') { targetColor = 0xf59e0b; targetEmissive = 0xd97706; }
      else { targetColor = 0x6366f1; targetEmissive = 0x4338ca; }
    }
...
export default function App() {
  ...
  const runAnalysis = async () => {
    ...
    setAnimatedScore(0);
    try {
      setActiveStep(0); await new Promise(r => setTimeout(r, 800));
      setActiveStep(1); await new Promise(r => setTimeout(r, 1200));
      setActiveStep(2); await new Promise(r => setTimeout(r, 1500));
      setActiveStep(3);
      let parsedResult;
      try {
      ...
      setResult({
        verdict: "ERROR",
        score: 0,
        explanation: `System fault: ${error.message}`,
        citations: ["System Diagnostics"]
      });
    } finally {
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        <nav className="backdrop-blur-xl bg-white/40 dark:bg-slate-900/60 border-b border-white/20 dark:border-white/5 sticky top-0 z-50 px-6 py-4 flex justify-between items-center transition-all">
          ...
          <div className="flex items-center gap-6">
            ...
            <button onClick={toggleTheme} className="w-10 h-10 rounded-full bg-white/50 dark:bg-white/10 border border-slate-200 dark:border-white/5 flex items-center justify-center hover:scale-105 active:scale-95 transition-all shadow-sm backdrop-blur-md">
        ...
        <main className="flex-grow p-4 md:p-6 lg:p-8 w-full max-w-[1600px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
            ...
            <div className="backdrop-blur-2xl bg-white/60 dark:bg-slate-900/50 border border-white/40 dark:border-white/10 p-6 md:p-8 rounded-[2rem] shadow-2xl transition-all relative overflow-hidden">
              ...
              <div className="relative z-10 space-y-5">
                ...
                <div className="relative group">
                  <textarea value={claimText} onChange={(e) => setClaimText(e.target.value)} disabled={isAnalyzing} className="w-full bg-white/50 dark:bg-black/40 border border-slate-200 dark:border-white/10 rounded-2xl p-5 text-sm md:text-base outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all min-h-[140px] resize-none placeholder-slate-400 dark:placeholder-slate-500 shadow-inner disabled:opacity-50" placeholder="E.g., 'Drinking hot lemon water cures cancer...'"/>
                  <div className="absolute right-3 bottom-3 flex gap-2">
                    <button className="w-10 h-10 rounded-xl bg-white/80 dark:bg-white/10 border border-slate-200 dark:border-white/5 flex items-center justify-center text-slate-500 dark:text-slate-400 hover:text-indigo-500 dark:hover:text-indigo-400 transition-colors shadow-sm backdrop-blur-sm" title="Upload Evidence (OCR)"><Camera className="w-4 h-4" /></button>
                    <button onClick={runAnalysis} disabled={!claimText.trim() || isAnalyzing} className="px-6 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-400 dark:disabled:bg-slate-700 text-white rounded-xl font-bold text-sm uppercase tracking-widest shadow-lg shadow-indigo-600/30 transition-all flex items-center gap-2 active:scale-95">
                  ...
                  <div className="p-6 md:p-8 space-y-6">
                    <div className="flex flex-col md:flex-row justify-between items-start gap-6">
                      <div className="space-y-3 flex-grow">
                        <div className="flex items-center gap-3">
                          <span className={`px-3 py-1 rounded-md text-[10px] font-black uppercase tracking-widest border ${getVerdictColor(result.verdict)}`}>{result.verdict}</span>
                        </div>
                      ...
                      <div className="flex flex-col items-center justify-center p-4 bg-white/50 dark:bg-black/30 rounded-2xl border border-slate-200 dark:border-white/5 min-w-[120px]">
                        ...
                        <div className="text-4xl font-black tabular-nums" style={{ color: result.score > 60 ? '#10b981' : result.score > 30 ? '#f59e0b' : '#e11d48' }}>{animatedScore}</div>
                      </div>
                    </div>
                    <div className="p-5 bg-white/40 dark:bg-white/5 rounded-2xl border border-slate-200/50 dark:border-white/5">
                      <p className="text-sm md:text-base text-slate-700 dark:text-slate-300 leading-relaxed font-medium">{result.explanation}</p>
                    </div>
          ...
          <div className="lg:col-span-5 space-y-6 lg:space-y-8 flex flex-col order-3">
            <div className="backdrop-blur-2xl bg-white/60 dark:bg-slate-900/50 border border-white/40 dark:border-white/10 p-6 md:p-8 rounded-[2rem] shadow-xl">
              <h3 className="text-xs font-black uppercase tracking-[0.2em] text-indigo-500 mb-6 flex items-center gap-2"><Activity className="w-4 h-4" /> Execution Pipeline</h3>
              <div className="space-y-4 relative">
                ...
                {PIPELINE_STEPS.map((step, idx) => {
                  ...
                  if (isActive) { iconColor = 'text-indigo-600 dark:text-indigo-400'; bgColor = 'bg-indigo-50 dark:bg-indigo-500/20'; borderColor = 'border-indigo-500/50'; containerClasses = 'scale-105 opacity-100 shadow-lg shadow-indigo-500/20'; }
                  else if (isPast || activeStep === 4) { iconColor = 'text-emerald-500'; containerClasses = 'scale-100 opacity-100'; }
                  return (
                    <div key={step.id} className={`relative z-10 flex items-center gap-4 p-3 rounded-2xl transition-all duration-500 border ${bgColor} ${borderColor} ${containerClasses} backdrop-blur-md`}>
                      ...
                      <div>
                        <h4 className={`text-xs font-bold uppercase tracking-widest ${isActive ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-700 dark:text-slate-200'}`}>{step.title}</h4>
            ...
            <div className="backdrop-blur-2xl bg-white/60 dark:bg-slate-900/50 border border-white/40 dark:border-white/10 p-6 md:p-8 rounded-[2rem] shadow-xl flex-grow">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xs font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400 flex items-center gap-2"><LineChart className="w-4 h-4" /> TRD Telemetry</h3>
              </div>
              <div className="h-40 w-full mb-6"><Line data={chartData} options={chartOptions} /></div>
